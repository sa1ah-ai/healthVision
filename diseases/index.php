<style>
    #diseasesAccordion button.btn.btn-block.text-left.font-weight-bolder:focus {
...
<div class="section py-5">
    <div class="container">
        <h3 class="text-center"><b>Diseases</b></h3>
        <hr>
        <div class="row justify-content-center">
            <div class="col-lg-8 col-md-10 col-sm-12 col-sm-12 mb-3">
                <div class="input-group input-group-lg" id="search-field">
                    <input type="search" class="form-control form-control-lg  rounded-pill" aria-label="Search Diseases Field" id="search" placeholder="Search Diseases here">
        ...
        <div class="accordion" id="diseasesAccordion">
            <?php
            $diseases = $conn->query("SELECT * FROM `diseases` where delete_flag = 0 and `status` = 1 order by `name` asc");
            while ($row = $diseases->fetch_assoc()):
            ?>
                <div class="card mb-0 rounded-0">
                    <div class="card-header" id="diseases<?= $row['id'] ?>">
                        <h2 class="mb-0">
                    ...
                    <div id="collapse<?= $row['id'] ?>" class="collapse diseases_collapse" aria-labelledby="diseases<?= $row['id'] ?>" data-parent="#diseasesAccordion">
                        <div class="card-body">
...
<script>
    $(function() {
        $('.diseases_collapse').on('show.bs.collapse', function() {
            var card = $(this).closest('.card')
            var icon = card.find('.collapse-icon')
            icon.removeClass('fa-plus').addClass('fa-minus')
        })
        $('.diseases_collapse').on('hide.bs.collapse', function() {
            var card = $(this).closest('.card')
            var icon = card.find('.collapse-icon')
        ...
        $('#search').on('input', function() {
            ...
            $('#diseasesAccordion .card').each(function() {
